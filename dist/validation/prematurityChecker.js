...
exports.checkPrematurity = checkPrematurity;
exports.formatPrematurityCheck = formatPrematurityCheck;
 ...
 */
function checkPrematurity(proposal, alternativeAnalysis) {
    try {
        const signals = [];
 ...
 */
function checkBlockingDependencies(proposal) {
    const signals = [];
    ...
    if (!(0, decisionClassifier_1.isTCBPath)(proposal.targetFile)) {
        signals.push({
 ...
 */
function checkAlternatives(proposal, alternativeAnalysis) {
    const signals = [];
    ...
    const weakRejections = alternativeAnalysis.alternatives.filter(a => a.rejectionReason.includes('fits the specific context') ||
 ...
 */
function checkAssumptions(proposal) {
    const signals = [];
 ...
 */
function generatePrematurityReason(signals, confidence) {
 ...
 */
function generateDeferralRecommendation(proposal, signals) {
 ...
 */
function formatPrematurityCheck(check) {
    const lines = [];
    lines.push('═══════════════════════════════════════════════════════════');
