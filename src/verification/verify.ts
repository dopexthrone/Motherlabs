...
import { JSONLRecord, JSONLLedger } from '../persistence/jsonlLedger'
import { contentAddress } from '../core/contentAddress'
import { GateDecision, requiresAuthorization } from '../core/gateDecision'
import { checkEffectBounds, EffectType } from '../core/effects'
...
/**
 * Check names for the 7 verification layers
 */
export type VerificationCheck =
  | 'envelope_schema'      // Layer 1: Valid record envelope
  | 'seq_increment'        // Layer 2: seq strictly increments by 1
  | 'prev_hash_chain'      // Layer 3: prev_hash correctness
  | 'record_hash_verify'   // Layer 4: record_hash recomputation
  | 'record_schema'        // Layer 5: Record-type specific schema
  | 'gate_authorization'   // Layer 6: Gate authorization for protected ops
  | 'effect_bounds'        // Layer 7: Effect bounds checking

/**
 * Verify entire ledger with 7-layer fail-closed verification
 */
export function verifyLedger(records: JSONLRecord[]): VerificationResult {
 ...
 */
function verifyEnvelopeSchema(record: JSONLRecord): string[] {
  const errors: string[] = []
 ...
 */
function verifyRecordSchema(record: JSONLRecord): string[] {
  const errors: string[] = []
  ...
  switch (record.record_type) {
    ...
    case 'GATE_DECISION':
      const decision = record.record as Partial<GateDecision>
      if (!decision.gate_type) errors.push('GATE_DECISION requires gate_type')
      if (!decision.decision || !['ALLOW', 'DENY'].includes(decision.decision)) {
        errors.push('GATE_DECISION decision must be ALLOW or DENY')
      }
      if (!decision.scope) errors.push('GATE_DECISION requires scope')
      if (!decision.authorizer) errors.push('GATE_DECISION requires authorizer')
      break

    case 'EVIDENCE_ARTIFACT':
      const artifact = record.record as Record<string, unknown>
      if (!artifact.artifact_id) errors.push('EVIDENCE_ARTIFACT requires artifact_id')
      if (!artifact.evidence_kind) errors.push('EVIDENCE_ARTIFACT requires evidence_kind')
      if (artifact.payload === undefined) errors.push('EVIDENCE_ARTIFACT requires payload')
      break
 ...
 */
function verifyGateAuthorization(records: JSONLRecord[]): VerificationError[] {
  const errors: VerificationError[] = []
  const gateDecisions: Map<string, GateDecision[]> = new Map()

  // First pass: collect all gate decisions
  for (const record of records) {
    if (record.record_type === 'GATE_DECISION') {
      const decision = record.record as GateDecision
  ...
  for (const record of records) {
    if (record.record_type === 'CHANGE_APPLIED') {
      const change = record.record as { proposal_id?: string }
      if (change.proposal_id) {
        const decisions = gateDecisions.get(change.proposal_id) || []
        const hasAllow = decisions.some(
          d => d.gate_type === 'change_application' && d.decision === 'ALLOW'
        )
        if (!hasAllow) {
          errors.push({
            seq: record.seq,
    ...
    if (record.record_type === 'PROPOSAL_ADMITTED') {
      const proposal = record.record as { proposal_id?: string }
      if (proposal.proposal_id) {
        const decisions = gateDecisions.get(proposal.proposal_id) || []
        const hasAllow = decisions.some(
          d => d.gate_type === 'proposal_admission' && d.decision === 'ALLOW'
        )
        if (!hasAllow) {
          errors.push({
            seq: record.seq,
 ...
 */
function verifyEffectBounds(records: JSONLRecord[]): VerificationError[] {
  const errors: VerificationError[] = []

  for (const record of records) {
    if (record.record_type === 'CHANGE_APPLIED') {
      const change = record.record as {
        granted_effects?: EffectType[]
        exercised_effects?: EffectType[]
      }

      if (change.granted_effects && change.exercised_effects) {
        const bounds = checkEffectBounds(change.granted_effects, change.exercised_effects)
        if (!bounds.valid) {
          errors.push({
            seq: record.seq,
            check: 'effect_bounds',
 ...
 */
export function verifyLedgerFromFile(filepath: string): VerificationResult {
  try {
