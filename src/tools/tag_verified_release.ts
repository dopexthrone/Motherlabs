...
function parseMarkdownForVerification(content: string): {
  ...
  return { verified, os, node_version, npm_version };
}
...
async function tagVerifiedRelease(options: TagVerifiedOptions): Promise<void> {
  const { release, threshold, date, dryRun } = options;
  ...
  // Load and count verified reports
  const reports = await loadVerifiedReports(release);
  const verifiedCount = reports.length;

  // Check threshold
  if (verifiedCount < threshold) {
    console.error(`THRESHOLD_NOT_MET: need ${threshold} verified report(s); have ${verifiedCount}`);
    process.exit(2);
  }
  ...
  const tagMessage = `Verified by ${verifiedCount} independent report(s); see artifacts/verifier_reports/INDEX.md`;
  ...
  console.log(`  Verifiers: ${verifiedCount}`);
...
function parseArgs(
  args: string[]
): { release: string; threshold: number; date?: string; dryRun: boolean } | null {
  ...
  if (date !== undefined) {
    return { release, threshold, date, dryRun };
  }
  return { release, threshold, dryRun };
}
...
function printHelp(): void {
  ...
  console.log('');
  console.log('Options:');
  console.log('  --threshold <n>    Minimum verified reports required (default: 1)');
