...
import { DecisionType, getTCBClassification } from '../core/decisionClassifier'
import type { ImprovementProposal } from '../selfbuild/proposer'
import type { PrematurityCheck } from './prematurityChecker'

/**
 * Gate requirement specification
 */
...
/**
 * Full gate elevation result
 */
export type GateElevation = {
...
/**
 * Additional requirements beyond gates
 */
 ...
 */
const GATE_DEFINITIONS: Record<string, { name: string; purpose: string }> = {
  schema_validation: {
...
export function determineGateRequirements(
  ...
  try {
    ...
    let level: 'standard' | 'elevated' | 'maximum'
    ...
    const additionalRequirements = buildAdditionalRequirements(
      level,
      proposal,
      isArchitectural,
      prematurityCheck
    ...
    const elevationReason = generateElevationReason(
      level,
      decisionType,
      tcbClass,
      isArchitectural
 ...
 */
function detectArchitecturalChange(proposal: ImprovementProposal): boolean {
 ...
 */
function buildGateRequirements(
  level: 'standard' | 'elevated' | 'maximum',
  proposal: ImprovementProposal,
  tcbClass: string
): GateRequirement[] {
 ...
 */
function generateElevationReason(
  level: 'standard' | 'elevated' | 'maximum',
  decisionType: DecisionType,
  tcbClass: string,
  isArchitectural: boolean
 ...
 */
export function checkGatesSatisfied(
  elevation: GateElevation,
  gateResults: Array<{ gateName: string; passed: boolean }>
): { satisfied: boolean; failedGates: string[] } {
 ...
 */
export function checkAdditionalRequirementsSatisfied(
  elevation: GateElevation
 ...
 */
export function formatGateElevation(elevation: GateElevation): string {
  const lines: string[] = []
 ...
 */
export function getElevationSummary(elevation: GateElevation): string {
