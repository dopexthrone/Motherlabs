 ...
 */
export function checkAxiomViolations(code: string): AxiomCheckResult {
 ...
 */
export function getAxiomViolationSummary(result: AxiomCheckResult): string {
 ...
 */
export function formatAxiomViolations(violations: AxiomViolation[]): string {
  if (violations.length === 0) {
    return 'No axiom violations detected'
  }

  return violations.map(v =>
    `[AXIOM ${v.axiom}] ${v.severity.toUpperCase()}: ${v.name}\n  ${v.message}${v.evidence ? `\n  Evidence: ${v.evidence}` : ''}`
  ).join('\n\n')
}
