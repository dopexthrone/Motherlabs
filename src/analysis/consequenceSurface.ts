...
/**
 * Consequence Surface - What a decision enables and forbids
 * Per DECISION_PHILOSOPHY.md Step 2
 */
...
/**
 * Full consequence analysis with context
 */
export type ConsequenceAnalysis = {
  ...
  reversibilityAssessment: {
    canRevert: boolean
    revertCost: 'trivial' | 'moderate' | 'expensive' | 'impossible'
    revertMethod?: string
  }
}
 ...
 */
const TCB_CONSEQUENCE_PATTERNS: Record<string, {
  enables: string[]
  forbids: string[]
  assumptions: string[]
  validationCriteria: string[]
 ...
 */
const CHANGE_TYPE_CONSEQUENCES: Record<string, {
  enables: string[]
  forbids: string[]
}> = {
  'add_function': {
 ...
 */
export function generateConsequenceSurface(
  proposal: ImprovementProposal
): Result<ConsequenceAnalysis, Error> {
  try {
    const surface: ConsequenceSurface = {
      enables: [],
      forbids: [],
      assumptions: [],
      validationCriteria: []
 ...
 */
function analyzeTCBImpact(filepath: string): ConsequenceSurface | null {
 ...
 */
function analyzeChangeTypeImpact(
  changeType: 'add_function' | 'modify_function' | 'add_test' | 'refactor'
): { enables: string[]; forbids: string[] } {
 ...
 */
function analyzeCodeImpact(code: string): ConsequenceSurface {
  const consequences: ConsequenceSurface = {
    enables: [],
    forbids: [],
    assumptions: [],
    validationCriteria: []
...
function assessReversibility(
  proposal: ImprovementProposal,
  surface: ConsequenceSurface
): ConsequenceAnalysis['reversibilityAssessment'] {
  const tcbClass = getTCBClassification(proposal.targetFile)
 ...
 */
export function formatConsequenceSurface(analysis: ConsequenceAnalysis): string {
  const lines: string[] = []
