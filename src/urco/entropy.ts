...
import { EntropyBreakdown, NodeEntropy, Entity, Action, MissingVar, Contradiction } from './types'
 ...
 */
function computeUnknowns(data: NodeData): number {
  const requiredSlots = ['goal', 'inputs', 'outputs', 'constraints', 'acceptanceCriteria', 'invariants']

  let missing = 0

  // Check goal (derived from text presence)
  if (!data.text || data.text.length < 10) missing++

  // Check other slots
  if (!data.inputs || (Array.isArray(data.inputs) && data.inputs.length === 0)) missing++
  if (!data.outputs || (Array.isArray(data.outputs) && data.outputs.length === 0)) missing++
  if (!data.constraints || (Array.isArray(data.constraints) && data.constraints.length === 0)) missing++
  if (!data.acceptanceCriteria || (Array.isArray(data.acceptanceCriteria) && data.acceptanceCriteria.length === 0)) missing++
  if (!data.invariants || (Array.isArray(data.invariants) && data.invariants.length === 0)) missing++
 ...
 */
function computeAmbiguity(data: NodeData): number {
 ...
 */
function computeContradiction(contradictions: Contradiction[]): number {
 ...
 */
function computeSpecificityDeficit(data: NodeData): number {
  let presentCount = 0
  const maxPoints = 4
 ...
 */
function computeDependencyUncertainty(data: NodeData): number {
  const text = data.text
 ...
 */
function computeVerifiabilityDeficit(data: NodeData): number {
  const text = data.text
...
export function computeEntropy(
  ...
  const value = clamp01(
    WEIGHTS.unknowns * breakdown.unknowns +
 ...
 */
export function entropyImproved(before: number, after: number, minImprovement: number = 0.15): boolean {
